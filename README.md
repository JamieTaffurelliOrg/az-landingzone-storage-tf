...
|------|-------------|------|---------|:--------:|
| <a name="input_boot_diagnostic_storage_account"></a> [boot\_diagnostic\_storage\_account](#input\_boot\_diagnostic\_storage\_account) | Deploy a storage account for boot diagnostics on VMs | <pre>object(<br>    {<br>      name                       = optional(string)<br>      resource_group_name        = optional(string)<br>      default_action             = optional(string, "Deny")<br>      ip_rules                   = optional(list(string), [])<br>      virtual_network_subnet_ids = optional(list(string), [])<br>    }<br>  )</pre> | `{}` | no |
