...
|------|-------------|------|---------|:--------:|
| <a name="input_boot_diagnostic_storage_accounts"></a> [boot\_diagnostic\_storage\_accounts](#input\_boot\_diagnostic\_storage\_accounts) | Deploy a storage account for boot diagnostics on VMs | <pre>list(object(<br>    {<br>      name                       = string<br>      resource_group_name        = string<br>      default_action             = optional(string, "Deny")<br>      ip_rules                   = optional(list(string), [])<br>      virtual_network_subnet_ids = optional(list(string), [])<br>    }<br>  ))</pre> | `[]` | no |
